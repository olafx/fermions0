...
tf = 1.25*np.pi/(Om**2+nu**2)**.5 # final time
...
dphidtau_target = (8/3/(xi := r/a), 4/3/xi)
...
if plot_3D:
  ...
  lim = (-(_ := max(np.max(np.abs(path[:,i])/a) for i in range(3))*1.1), _)
  sm = plt.cm.ScalarMappable(Normalize(path[0,0]*om0, path[-1,0]*om0), cmap)
  def gen_lc(i, j):
    lc, colors = [], []
    for part_i, (left, right) in enumerate(part_ids):
      lc += [np.column_stack((path[left:right,i]/a, path[left:right,j]/a))]
      colors += [color(part_i)]
  ...
  plt.colorbar(sm, ax=plt.gca(), fraction=.046, pad=.04).set_label(R'$\tau$')
  ...
  plt.tight_layout()
  if plot_save: plt.savefig(Path('out')/f'Sch_H_Pauli_{orbital}_rate.png', dpi=1000, bbox_inches='tight')
